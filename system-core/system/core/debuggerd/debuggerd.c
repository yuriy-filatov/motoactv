...
void dump_crash_banner(int tfd, unsigned pid, unsigned tid, int sig, char* data)
{
    _LOG(tfd, false,
         "*** *** *** *** *** *** *** *** *** *** *** *** *** *** *** ***\n");
    dump_build_info(tfd);
    _LOG(tfd, false, "pid: %d, tid: %d  >>> %s <<<\n",
         pid, tid, strlen(data) ? data : "UNKNOWN");
                              ...
                              int signal, char* data)
{
    ...
    dump_crash_banner(fd, pid, tid, signal, data);
...
static void handle_crashing_process(int fd)
{
    ...
    /* Reading /proc/%d/cmdline here instead of later, just in case /proc/%d goes POOF
       ... like it has been in some cases.  This at least tells us what process it was.
       Moved here by Rob Stoddard
    */
    sprintf(data, "/proc/%d/cmdline", cr.pid);
    fp = fopen(data, "r");
    if(fp) {
    ...
    if(x == NULL) for(n = 0; n < 1024; n++) data[n] = 0;
    ...
    for(;;) {
        ...
        if(WIFSTOPPED(status)){
            ...
            case SIGSTKFLT: {
                ...
                need_cleanup = engrave_tombstone(cr.pid, tid, debug_uid, n, data);
                kill(tid, SIGSTOP);
                goto done;
            }
...
int main()
{
    ...
    for(;;) {
        ...
        /* Fork so we can handle process deaths concurrently by Rob Stoddard */
	if(fork() == 0)
	{
