...
static int alias_part_count = 0;

void add_mmc_alias(char *dev_name, char *dev_alias)
{
    if (dev_alias[0] != '\0') {
        if (alias_part_count < MAX_MMC_PARTITIONS) {
            char dev_path[PATH_LEN]={'\0'};
            sprintf(dev_path, "/dev/block/%s", dev_name);

            int i;
            for (i = 0; i < alias_part_count; i++) {
                if (strcmp(dev_name, mmc_part_map[i].name) == 0)
                    break;
            }

            if (i == alias_part_count) {
                char dev_link[PATH_LEN]={'\0'};
                unsigned uid = 0;
                unsigned gid = 0;
                mode_t mode = 0;
                sprintf(dev_link, "/dev/block/%s", dev_alias);
                strncpy(mmc_part_map[alias_part_count].name,
                        dev_name, NAME_LEN);
                strncpy(mmc_part_map[alias_part_count].alias,
                        dev_alias, ALIAS_LEN);
                alias_part_count++;
                ...
                if (!symlink(dev_path, dev_link)) {
                    ...
                    ERROR("Create mmc alias link error!\n");
                }
            }
        } else {
...
static void find_mmc_partitions(void)
{
    ...
    *data_end = '\0';
    while (data_ptr < data_end) {
        ...
        if (r == 5 && !strncmp(dev_name, "mmc", 3)) {
            add_mmc_alias(dev_name, dev_alias);
        }
...
char * mmc_alias_to_name(const char *alias)
{
    int n;
    if (alias_part_count == 0) {
        find_mmc_partitions();
    }
    for (n = 0; n < alias_part_count; n++) {
        if (!strcmp(alias, mmc_part_map[n].alias)) {
            return mmc_part_map[n].name;
        }
    }
    return NULL;
}

/* If userdata hasn't been found yet, wait for it to be created.
 * Since userdata is the last partition in the memory map, all
 * other partitions should be available by the time userdata is found.
 */
void wait_for_partitions()
{
    int userdata_found = 0;
    struct stat info;
    int fd_count = 1;
    ...
    while (1) {
        ...
        userdata_found = mtd_name_to_number("userdata") >= 0 ? 1 : 0;
        if (!userdata_found) {
            find_mmc_partitions();
            /* Check to make sure that the device exists since the symlink
             * might be created before the /dev/block/mmcblk1pX device
             */
            INFO("userdata device: %s\n", USERDATA_DEVICE);
            userdata_found = stat(USERDATA_DEVICE, &info) == 0 ? 1 : 0;
        }
        ...
        if (nr < 0) {
            ERROR("poll returned %d (%s)\n", nr, strerror(errno));
            continue;
        }
        else if (nr == 0) {
            ERROR("Timed out waiting for partitions to be found, attempting boot anyway\n");
            break;
        }
