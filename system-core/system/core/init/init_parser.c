...
static void parse_line_service(struct parse_state *state, int nargs, char **args)
{
    ...
    int i, kw, kw_nargs;
    int keycode, fd;
    int keycode_size = sizeof(keycode);
    char keycodes_prop_name[DT_PROP_NAME_MAX + 1] = DT_PREFIX;
    ...
    case K_group:
        ...
        break;
    case K_keycodes:
        if (DT_PROP_NAME_MAX >= strlen(DT_PREFIX) + strlen(DT_PATH_KEYPAD) +
                                    strlen(DT_PROP_KEYPAD_KEYCODES) +
                                    strlen(svc->name)) {
            svc->keycodes = 0;
            svc->nkeycodes = 0;

            strcat(strcat(strcat(keycodes_prop_name, DT_PATH_KEYPAD),
                DT_PROP_KEYPAD_KEYCODES), svc->name);

            if ((fd = open(keycodes_prop_name, O_RDONLY)) > 0) {
                while (keycode_size == sizeof(keycode)) {
                    keycode_size = read(fd, &keycode, sizeof(keycode));

                    if ((keycode_size == sizeof(keycode)) &&
                        (keycode > KEY_RESERVED) && (keycode < KEY_UNKNOWN)) {
                        svc->keycodes = realloc(svc->keycodes,
                            (svc->nkeycodes + 1)*sizeof(svc->keycodes[0]));

                        if (!svc->keycodes) {
                            svc->nkeycodes = 0;
                            break;
                       } else {
                            svc->keycodes[svc->nkeycodes] = keycode;
                            svc->nkeycodes++;
                        }
                    } else {
                        if (keycode_size) {
