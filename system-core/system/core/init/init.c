...
static int set_init_properties_action(int nargs, char **args)
{
     ...
     */
    /* IKXGII-706 - If the ro.serialno is written here, it can be programmed again, hence we
	   can remove this option and program later. Later, we can remove the property androidboot.serialno
       from the bootargs itself. The ro.serialno would come from another service named "serialno_tracker"
    */
    if(serialno[0])
