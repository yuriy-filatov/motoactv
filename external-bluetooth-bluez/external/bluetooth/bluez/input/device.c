...
static struct input_device *find_device(const bdaddr_t *src, const bdaddr_t *dst);
								...
								bdaddr_t *src, bdaddr_t *dst)
{
	...
	if (status) {
		error("Encryption failed: %s(0x%x)",
				...
				emit_property_changed(idev->conn, idev->path, INPUT_DEVICE_INTERFACE,
								"Hid_Auth_Failure", DBUS_TYPE_BOOLEAN, &auth_failed);
		goto failed;
	}
