				...
				DBusMessage *msg, void *user_data)
{
	...
	/* UUIDsLE */
	str = g_new0(char *, g_slist_length(device->uuids_le) + 1);
	for (i = 0, l = device->uuids_le; l; l = l->next, i++)
		str[i] = l->data;
	dict_append_array(&dict, "UUIDsLE", DBUS_TYPE_STRING, &str, i);
	g_free(str);
...
static DBusMessage *get_service_attribute_value_reply(DBusMessage *msg,
							DBusConnection *conn,
							sdp_data_t *attr)
{
	DBusMessage *reply;
...
int read_profile_version(struct btd_device *device, const char *pattern)
{
	sdp_record_t *rec;
	int att_value = 0;
	...
	sdp_list_t *profDescSeq = NULL;
	sdp_get_profile_descs(rec, &profDescSeq);
	if (profDescSeq == NULL) {
		error("Profile Des List is NULL");
		return -1;
	}
	sdp_profile_desc_t *desc = (sdp_profile_desc_t *)(profDescSeq->data);
	if (desc == NULL) {
		error("Profile Desc is NULL");
		return -1;
	}

	att_value = desc->version;
	DBG ("Profile %s version is %d", pattern, att_value);
	return att_value;

}

static DBusMessage *get_profile_version(DBusConnection *conn,
						DBusMessage *msg,
						void *user_data)
{
	struct btd_device *device = user_data;
	...
	att_value = read_profile_version(device, pattern);
	if (att_value == -1)
		goto fail;
	DBG ("Profile %s version is %d", pattern, att_value);
	dbus_message_append_args(reply,
		DBUS_TYPE_UINT16, &att_value,
		DBUS_TYPE_INVALID);
	return reply;
fail:
...
int read_supported_feature_value(struct btd_device *device,
					const char *pattern, uint16_t attrId)
{
	sdp_record_t *rec;
	...
	sdp_list_t *attlist = (sdp_list_t *)rec->attrlist;
	for (; attlist; attlist = attlist->next) {
		sdp_data_t *dat = (sdp_data_t *)attlist->data;
		if (dat->attrId == attrId) {
			int *t = (int *)(&dat->val);
			att_value = *t;
			DBG ("Supported feature %x is ", dat->attrId);
			DBG ("Value is %d", *t);
			att_value = *t;
			return att_value;
		}
...
static DBusMessage *get_supported_feature_value(DBusConnection *conn,
						DBusMessage *msg,
						void *user_data)
{
	struct btd_device *device = user_data;
	...
	att_value = read_supported_feature_value(device, pattern, attrId);
	if (att_value == -1)
		goto fail;

	DBG ("Supported feature value is %d", att_value);
	dbus_message_append_args(reply,
		DBUS_TYPE_UINT16, &att_value,
		DBUS_TYPE_INVALID);
	return reply;
...
static GDBusMethodTable device_methods[] = {
	...
	{ "GetSupportedFeatureValue",  "sq", "i",       get_supported_feature_value},
	{ "GetProfileVersion",	"s",	"i",		get_profile_version},
	{ }
};
...
void device_remove_connection(struct btd_device *device, DBusConnection *conn, uint8_t reason) //added disconnection reason
{
    uint32_t discReason = (uint32_t)reason;
	if (!device->connected) {
		char addr[18];
		ba2str(&device->bdaddr, addr);
...
void device_probe_primary(struct btd_device *device, GSList *profiles)
{
	GSList *list;
	char addr[18];
	int err, pos = 0;
	...
	DBG("Probing primary services for %s", addr);
	...
	for (list = device_drivers; list; list = list->next) {
		...
		if (err < 0) {
			error("%s primary probe failed for device %s",
							driver->name, addr);
...
add_uuids:
	...
	for (list = profiles; list; list = list->next, pos++) {
		GSList *l = g_slist_find_custom(device->uuids_le, list->data,
						(GCompareFunc) strcasecmp);
		if (l)
			continue;

		device->uuids_le = g_slist_insert(device->uuids_le,
						g_strdup(list->data),
						pos);
	}
}
...
static void services_changed_primary(struct btd_device *device)
{
	DBusConnection *conn = get_dbus_connection();
	char **uuids;
	GSList *l;
	...
	uuids = g_new0(char *, g_slist_length(device->uuids_le) + 1);
	for (i = 0, l = device->uuids_le; l; l = l->next, i++)
		uuids[i] = l->data;

	emit_array_property_changed(conn, device->path, DEVICE_INTERFACE,
					"UUIDsLE", DBUS_TYPE_STRING, &uuids, i);
...
static void primary_cb(GSList *services, guint8 status, gpointer user_data)
{
	...
	device_probe_primary(device, uuids);

	services_changed_primary(device);
