...
static int QueryingForDeviceInRange=0;
static int attemptsPending=0,requestedFromApp=0;
static const bdaddr_t Appaddress;
...
void adapter_querry_names_rsp(struct btd_adapter *adapter, struct btd_device *device, uint8_t status)
{
	if(!status)
	{
			QueryingForDeviceInRange = 0;
			attemptsPending = 0;
			requestedFromApp = 0;
		
		/*Oh device is in range !*/
		/*emit property to upper layer*/
		DBG("device is in range !");
		emit_property_changed(connection, adapter->path,
					ADAPTER_INTERFACE, "DeviceInRange",
					DBUS_TYPE_BOOLEAN, "Dummy");
	}
	else
	{
		if(--attemptsPending)
		{
			...
			if (!adapter)
				return;
			DBG("Requesting Remote Name again !");
			if ( !requestedFromApp )
			{
				device_get_address(device, &bdaddr);
				err = adapter_ops->resolve_name(adapter->dev_id, &bdaddr);
				if (err)
			...
			{
				adapter_ops->resolve_name(adapter->dev_id, &Appaddress);
				if (err)
					error("Unable to send HCI remote name req: %s (%d)",
							strerror(errno), errno);
			}
		...
		{
			DBG("Attempts Finished !");	
			QueryingForDeviceInRange = 0;
			attemptsPending = 0;
			requestedFromApp = 0;
			DBG("device is not in range !");
			emit_property_changed(connection, adapter->path,
					ADAPTER_INTERFACE, "DeviceNotInRange",
					DBUS_TYPE_BOOLEAN, "Dummy");
		}
	}	
}
int adapter_QueryingForDeviceInRange(bdaddr_t *bdaddr)
{
	DBG("QueryingForDeviceInRange! %d",QueryingForDeviceInRange);
	if ( QueryingForDeviceInRange && bdaddr != NULL && !bacmp(bdaddr,&Appaddress))
	{
		DBG("This is the device we are interested in!");
		return 1;
	}
	return 0;
	//return QueryingForDeviceInRange;
}
...
static int start_le_discovery(struct btd_adapter *adapter)
{
	int err, type;
	DBG("satish in adapter ");
	/* Do not start if suspended */
	if (adapter->state & STATE_SUSPENDED)
		return 0;
						...
						DBusMessage *msg, void *data)
{
	...
	adapter->oor_devices = NULL; 
	 ...
	 err = start_le_discovery(adapter);
    //   err = adapter_ops->start_scanning(adapter->dev_id);
						...
						DBusMessage *msg, void *data)
{
	...
	if (adapter->stop_discov_id > 0) {
		g_source_remove(adapter->stop_discov_id);
		adapter->stop_discov_id = 0;
	}	
	adapter_ops->stop_scanning(adapter->dev_id);
	return dbus_message_new_method_return(msg);
}
...
static DBusMessage *check_device_in_Range(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	struct btd_adapter *adapter = data;
	const gchar *bdaddr,*sender = dbus_message_get_sender(msg);
	struct btd_device *device;
	uint32_t attempts;

	if( !QueryingForDeviceInRange) {
		if (!dbus_message_get_args(msg, NULL,
				DBUS_TYPE_STRING, &bdaddr,
				DBUS_TYPE_UINT32, &attempts,
				DBUS_TYPE_INVALID))
			return btd_error_invalid_args(msg);

		if (check_address(bdaddr) < 0)
			return btd_error_invalid_args(msg);
		DBG("attempts = %d!", attempts);
		attemptsPending = attempts;
		if(attemptsPending > 0)
		{
			QueryingForDeviceInRange = 1;
			requestedFromApp = 1;
			DBG("bdaddr = %s!", bdaddr);
			str2ba(bdaddr, &Appaddress);
			adapter_ops->resolve_name(adapter->dev_id, &Appaddress);
		}
	}
	return dbus_message_new_method_return(msg);
                                        ...
                                        DBusMessage *msg, void *data)
{
	...
	err = adapter_ops->write_link_timeout(index,
					&bdaddr, num_slots);
...
static GDBusMethodTable adapter_methods[] = {
    ...
    { "StartLEDiscovery",	"",	"",	adapter_start_le_discovery },
	{ "StopDiscovery",	"",	"",	adapter_stop_discovery,
						G_DBUS_METHOD_FLAG_ASYNC},
	{ "StopLEDiscovery",	"",	"",	adapter_stop_le_discovery,
						G_DBUS_METHOD_FLAG_ASYNC},									
	...
	{ "CancelDeviceCreation","s",	"",	cancel_device_creation,
						G_DBUS_METHOD_FLAG_ASYNC},						
	{ "CheckDeviceInRange","su",	"",	check_device_in_Range,
						G_DBUS_METHOD_FLAG_ASYNC},
...
int btd_adapter_stop(struct btd_adapter *adapter)
{
	...
	while (adapter->connections) {
		...
		adapter_remove_connection(adapter, device,0xff); //added disconnect reason; here 0xff dummy
	}
						...
						struct remote_dev_info *dev)
{
	...
	emit_device_found(adapter->path, paddr,
			...
			"RSSI", DBUS_TYPE_INT16, &rssi,
			"Name", DBUS_TYPE_STRING, &dev->name,
			"DeviceId", DBUS_TYPE_STRING, &dev->deviceId,
...
void adapter_update_device_from_info(struct btd_adapter *adapter,
					...
					GSList *services, int flags , const char *deviceid)
{
	struct remote_dev_info *dev;
	gboolean new_dev;
	...
	dev->rssi = rssi;
	if (deviceid) {
		dev->deviceId = g_strdup(deviceid);
	}
...
void adapter_update_found_devices(struct btd_adapter *adapter, bdaddr_t *bdaddr,
				...
				GSList *services, name_status_t name_status,const char *deviceid)
{
	struct remote_dev_info *dev;
	gboolean new_dev;
...
void adapter_remove_connection(struct btd_adapter *adapter,
						struct btd_device *device, uint8_t reason) //added disconnect reason
{
	bdaddr_t bdaddr;
