...
int btd_event_request_pin(bdaddr_t *sba, bdaddr_t *dba)
{
	...
	/*HardCode "0000" Pincode for non SSP security method*/
	//pinlen = read_pin_code(sba, dba, pin);
    pinlen = 4;
	strncpy(pin,"0000",pinlen);
	...
	while (len < eir_length - 1) {
		...
		case EIR_DEVICE_ID:
			for (  i = 0; i < field_len ; i++)
				deviceid.data[i] = eir_data[2 + i];
			memcpy(&data0, &deviceid.data[0], 4);
			memcpy(&data1, &deviceid.data[4], 4);

			str = g_try_malloc0(16);
			if (!str)
				return -EINVAL;
			/*Print the each 4 byte into 8 char wide Hex format string.
			* So 8 byte deviceid takes 16 symbols*/
			sprintf(str, "%.8x%.8x",g_ntohl(data0),g_ntohl(data1));
			eir->deviceId = str;
		}
...
void btd_event_advertising_report(bdaddr_t *local, le_advertising_info *info)
{
	...
	adapter_update_device_from_info(adapter, info->bdaddr, rssi,
					...
					eir_data.services, eir_data.flags,eir_data.deviceId);
...
void btd_event_querry_remote_name(bdaddr_t *local, bdaddr_t *peer, uint8_t status)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
		DBG("Entered btd_event_querry_remote_name");
		if (!get_adapter_and_device(local, peer, &adapter, &device, FALSE))
			return;
		if(adapter != NULL && device != NULL)
			adapter_querry_names_rsp(adapter,device,status);
}
int btd_QueryingForDeviceInRange(bdaddr_t *peer)
{
	DBG("Entered btd_QueryingForDeviceInRange");
	return adapter_QueryingForDeviceInRange(peer);
}
...
void btd_event_disconn_complete(bdaddr_t *local, bdaddr_t *peer, uint8_t reason)//added disconnect reason
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	...
	adapter_remove_connection(adapter, device,reason);//added disconnect reason
}
...
void btd_event_le_passkey_notify(int index, uint32_t *passkey)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	const gchar *path;
	...
	g_dbus_emit_signal(conn, path,
			ADAPTER_INTERFACE, "SmpPasskeyNotify",
			DBUS_TYPE_STRING, &smp_passkey,
			DBUS_TYPE_INVALID);
}
