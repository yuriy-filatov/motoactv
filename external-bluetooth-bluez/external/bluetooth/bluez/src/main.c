...
static void parse_config(GKeyFile *config)
{
	...
	str = g_key_file_get_string(config, "General", "ManufactureSpecific", &err);
	if (err) {
		DBG("%s", err->message);
		g_clear_error(&err);
	} else {
		DBG("ManufactureSpecific=%s", str);
		strncpy(main_opts.manufacturespecific, str,
					sizeof(main_opts.manufacturespecific));
		g_free(str);
	}	
