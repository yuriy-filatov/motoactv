...
static int add_map(sdp_session_t *session, svc_info_t *si)
{
	...
	sdp_list_t *aproto, *proto[3];
	sdp_record_t record;
	uint8_t chan = si->channel ? si->channel : 20;
	sdp_data_t *channel;
	uint8_t formats[] = { 0x0b };
	uint8_t nwid = si->network;
	/*Per MAP Spec 1.0 in SDP record, supported message types will have
	information on the SMS(CDMA/GSM) type on MSE,using network id supported messages
	types fields are changed,if not provided default value will be for SMS_GSM*/
	if (nwid == 2) {
		formats[0] = 0x0d;
	}
	uint8_t mas_instance[] = { 0x00 };
	void *dtds[sizeof(formats)], *values[sizeof(formats)];
	int i;
	uint8_t dtd = SDP_UINT8;
	sdp_data_t *sflist;
	sdp_data_t *sflist_mas;
	int ret = 0;
	...
	sdp_uuid16_create(&map_uuid, MAP_MAS_SVCLASS_ID);
	...
	sflist = sdp_data_alloc(dtd, formats);
	sdp_attr_add(&record, SDP_ATTR_MAP_SUPPORTED_REPOSITORIES, sflist);

	sflist_mas = sdp_data_alloc(dtd, mas_instance);
	sdp_attr_add(&record, SDP_ATTR_MAP_MAS_INSTANCE_ID, sflist_mas);

	sdp_set_info_attr(&record, "MAP MASname", 0, 0);
...
static int add_nap(sdp_session_t *session, svc_info_t *si)
{
	...
	add_lang_attr(&record);
	sdp_attr_add_new(&record, SDP_ATTR_SECURITY_DESC, SDP_UINT16,
		&security_desc);
	sdp_set_info_attr(&record, "Network Access Point Service", 0,
	"Personal Ad-hoc Network Service which provides access to a network");

	sdp_attr_add_new(&record, SDP_ATTR_NET_ACCESS_TYPE, SDP_UINT16,
		&net_access_type);
	sdp_attr_add_new(&record, SDP_ATTR_MAX_NET_ACCESSRATE, SDP_UINT32,
		&max_net_access_rate);
...
static int add_panu(sdp_session_t *session, svc_info_t *si)
{
	...
	sdp_set_info_attr(&record, "Personal Ad-hoc User Service", NULL,
...
static const char *remove_help =
	"Usage:\n"
	"\tremove serviceclassname\n";

/*
 * Search and remove for a specific SDP service
 */
static int do_remove(struct search_context *context)
{
	...
	for (; seq; seq = next) {
		...
		printf("Unregisted service record\n");
...
static int cmd_remove(int argc, char **argv)
{
	...
	for_each_opt(opt, remove_options, 0) {
		switch (opt) {
		default:
			printf("%s", remove_help);
			return -1;
		}
	}
...
static struct {
	...
	{ "remove",  cmd_remove,      "Remove local service by service name"},
