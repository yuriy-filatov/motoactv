...
static void append_char_dict(DBusMessageIter *iter, struct characteristic *chr)
{
	...
	ccc_desc = g_strdup_printf("CCC%04x%04x", chr->ccc_handle, chr->ccc_value);

	if(ccc_desc) {
		DBG("%s", ccc_desc);
		dict_append_entry(&dict, "CCCFG", DBUS_TYPE_STRING, &ccc_desc);
	}
...
static void attrib_disconnect(gpointer user_data)
{
	...
	emit_property_changed(gatt->conn, gatt->path, CHAR_INTERFACE,
			"Connected", DBUS_TYPE_BOOLEAN, &connected);

	/* Remote initiated disconnection only */
	g_attrib_unref(gatt->attrib);

	/* Destroy attrib and all its ref for remote disconnect only */
	g_attrib_destroy(gatt->attrib);
}
...
static void connect_cb(GIOChannel *chan, GError *gerr, gpointer user_data)
{
	...
	if (gerr) {
		...
		emit_property_changed(conn, gatt->path, CHAR_INTERFACE,
				"Connected", DBUS_TYPE_BOOLEAN, &connected);
		goto fail;
	}

	/* Listen mode: used for notification and indication */
	if (gatt->attrib != NULL && gatt->listen == TRUE) {
	...
	if(gatt->msg ) {
		DBG("gatt->msg is not NULL");
		DBusMessage *reply = dbus_message_new_method_return(gatt->msg);
		if (!reply)
			return NULL;
						...
						DBusMessage *msg, void *data)
{
	...
	DBG("register watcher for %s", path);
						...
						DBusMessage *msg, void *data)
{
	...
	DBG("unregister watcher for %s", path);
...
static DBusMessage *set_client_cfg(DBusConnection *conn, DBusMessage *msg,
			DBusMessageIter *iter, struct characteristic *chr)
{
	struct gatt_service *gatt = chr->prim->gatt;
	DBusMessageIter sub;
	...
	gatt_write_cmd(gatt->attrib, chr->ccc_handle, value, len, NULL, NULL);
					...
					DBusMessage *msg, void *data)
{
	...
	else if (g_str_equal("CCCFG", property))
		return set_client_cfg(conn, msg, &sub, chr);
...
static void update_char_val(guint8 status, const guint8 *pdu,
...
static void update_char_client_cfg(guint8 status, const guint8 *pdu,
					guint16 len, gpointer user_data)
{
	struct characteristic *dst_chr;
	struct query_data *current = user_data;
	...
	DBG(" status 0x%02x len = %d", status, len-1);
    if (conn == NULL)
		conn = get_dbus_connection();
	
	
	if (status == 0) {
		chr->ccc_value = (*(pdu + 2) << 8);
		chr->ccc_value = chr->ccc_value | *(pdu + 1);

		DBG("ccc desc value = 0x%04x", chr->ccc_value);

		chr->ccc_handle = current->handle;

		guint handle = current->chr->handle;

		GSList *lchr;

		/* search for matching char and update its ccc_value */
		lchr = g_slist_find_custom(prim->chars,
			GUINT_TO_POINTER(handle), characteristic_handle_cmp);

		if(lchr) {
			DBG("matching char found update the same");

			dst_chr = lchr->data;
			dst_chr->ccc_handle = current->handle;
			dst_chr->ccc_value = chr->ccc_value;
			emit_property_changed(conn, gatt->path, CHAR_INTERFACE,
		    "ClientCharDescHandleUpdated",
		    DBUS_TYPE_OBJECT_PATH,
		    &chr->path);
		} else {
			DBG("matching char not found");
		}


		store_attribute(gatt, current->handle,
				GATT_CLIENT_CHARAC_CFG_UUID,
				(void *)&chr->ccc_value, len - 1);
	} else if (status == ATT_ECODE_INSUFF_ENC) {
		GIOChannel *io = g_attrib_get_channel(gatt->attrib);
		...
		if (bt_io_set(io, BT_IO_L2CAP, NULL,
			...
			BT_IO_OPT_INVALID)) {
			gatt_read_char(gatt->attrib, current->handle, 0,
				update_char_client_cfg, current);
			return;
		}
	}
							...
							void *data)
{
	...
	qvalue->prim = prim;
	qvalue->chr = chr;
	qvalue->msg = dbus_message_ref(msg);

	gatt->attrib = g_attrib_ref(gatt->attrib);
	gatt_read_char(gatt->attrib, chr->handle, 0, update_char_val, qvalue);
...
static GDBusMethodTable char_methods[] = {
	...
	{ "ReadCharacteristic",	"",	"a(sv)", read_char,
						G_DBUS_METHOD_FLAG_ASYNC},
	{ }
};
...
static DBusMessage *connect_gatt(DBusConnection *conn, DBusMessage *msg,
							void *data)
{
	struct primary *prim = data;
	GError *gerr = NULL;

	DBG(" Connecting to GATT");
	...
	prim->gatt->conn = dbus_connection_ref(conn);
	prim->gatt->msg = dbus_message_ref(msg);

	return NULL;
}

static DBusMessage *disconnect_gatt(DBusConnection *conn, DBusMessage *msg,
							void *data)
{
	struct primary *prim = data;
	gboolean connected = FALSE;

	DBG(" Disconnecting GATT");

	if(prim && prim->gatt && prim->gatt->attrib) {
		g_attrib_unref(prim->gatt->attrib);
	}

	emit_property_changed(conn, prim->gatt->path, CHAR_INTERFACE,
			"Connected", DBUS_TYPE_BOOLEAN, &connected);
...
static GDBusMethodTable prim_methods[] = {
	...
	{ "ConnectGatt",	"",	"",	connect_gatt,
					G_DBUS_METHOD_FLAG_ASYNC	},
	{ "DisconnectGatt",	"",	"",	disconnect_gatt,
					G_DBUS_METHOD_FLAG_ASYNC	},
	{ }
};

static GDBusSignalTable prim_signals[] = {
	{ "PropertyChanged",	"sv"	},
	{ }
};
...
static GSList *register_primaries(struct gatt_service *gatt, GSList *primaries)
{
	...
	for (paths = NULL, l = primaries; l; l = l->next) {
		...
		g_dbus_register_interface(gatt->conn, prim->path,
				...
				prim_signals, NULL, prim, NULL);
		DBG("Registered: %s for uuid %s", prim->path, att->uuid);
