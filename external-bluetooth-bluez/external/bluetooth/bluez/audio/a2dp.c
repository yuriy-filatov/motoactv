...
#ifdef ENABLE_MP3_ITTIAM_CODEC
		codec_cap = (void *) cap->data;
		DBG("rfa0:=%u\n, media_type:=%u",codec_cap->rfa0,codec_cap->media_type);
		DBG("media_codec_type=%u\n, data[0]=%u",codec_cap->media_codec_type,
									codec_cap->data);

		if (codec_cap->media_codec_type != A2DP_CODEC_MPEG12 )
			continue;
#endif //ENABLE_MP3_ITTIAM_CODEC
	}
				...
				uint8_t *category, void *user_data)
{
	...
	for (codec_cap = NULL; caps; caps = g_slist_next(caps)) {
		...
		if (codec_cap->media_codec_type != A2DP_CODEC_BTAPTX)
			continue;
		...
		if (btaptx_cap->vender_id0 != BTAPTX_VENDER_ID0
			...
			||btaptx_cap->cap.media_codec_type != A2DP_CODEC_BTAPTX)
		{
			*err = AVDTP_UNSUPPORTED_CONFIGURATION;
			*category = AVDTP_MEDIA_CODEC;
			return FALSE;
...
static gboolean btaptx_getcap_ind(struct avdtp *session, struct avdtp_local_sep *sep, gboolean get_all,
...
#ifdef ENABLE_MP3_ITTIAM_CODEC
	mpeg_cap.layer = MPEG_LAYER_MP3; //MP3 codec supports only Layer3
#else
...
static gboolean a2dp_reconfigure(gpointer data)
{
	...
	if (posix_err < 0) {
		...
		if (avdtp_compare_sep_codecs(rsep, setup->rsep)) {
			posix_err = avdtp_set_configuration(setup->session, setup->rsep, lsep,
						setup->client_caps,
						&setup->stream);
		}
		else {
...
#ifdef ENABLE_CSR_APTX_CODEC
	int btaptx_srcs = 0, btaptx_sinks = 0;
#endif //ENABLE_CSR_APTX_CODEC
...
proceed:
	...
	if (source) {
		...
		for (i = 0; i < btaptx_srcs; i++)
			a2dp_add_sep(server, AVDTP_SEP_TYPE_SOURCE,
					A2DP_CODEC_BTAPTX, delay_reporting);
#endif
	}
				...
				struct avdtp_remote_sep *rsep)
{
	...
	for (; l != NULL; l = l->next) {
		...
		DBG("a2dp_get: locked %d, codec %d, stream %d, avdtp has stream %d",
						sep->locked, sep->codec , sep->stream,
						avdtp_has_stream(session, sep->stream));
	...
	error("a2dp_get: unable to find sep. return NULL");
	return NULL;
}
