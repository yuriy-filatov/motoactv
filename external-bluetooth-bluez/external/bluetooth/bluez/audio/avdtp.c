...
{
	if (!stream) {
		DBG("entered condition stream is null");
		return;
	}
	struct avdtp_local_sep *sep = stream->lsep;
					...
					struct discover_resp *resp, int size)
{
	...
	for (i = 0; i < sep_count; i++) {
		...
		DBG("seid %d type %d media %d in use %d sep %08x stream %08x",
				resp->seps[i].seid, resp->seps[i].type,
				resp->seps[i].media_type, resp->seps[i].inuse, sep, stream);

		if (!sep) {
			if (resp->seps[i].inuse && !stream) {
				DBG("seid %d in use, skipped", resp->seps[i].seid);
				continue;
                        }
			resp_found = 1;
...
struct avdtp_remote_sep *avdtp_stream_get_remote_sep(struct avdtp_stream *stream)
{
...
gboolean avdtp_compare_sep_codecs(struct avdtp_remote_sep *rsep_new,
				  struct avdtp_remote_sep *rsep_old)
{
	struct avdtp_service_capability *cap_new = rsep_new->codec;
	struct avdtp_service_capability *cap_old = rsep_old->codec;

	struct avdtp_media_codec_capability *codec_cap_new = (void *) cap_new->data;
	struct avdtp_media_codec_capability *codec_cap_old = (void *) cap_old->data;

	if (codec_cap_new->media_codec_type == codec_cap_old->media_codec_type) {
		return TRUE;
	}
