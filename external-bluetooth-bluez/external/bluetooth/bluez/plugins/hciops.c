			...
			GIOCondition cond, gpointer user_data)
{
	...
	if (cond & G_IO_NVAL) {
		cmd->cb(status, cmd->caller_data, cmd->src, cmd->dst);
		return FALSE;
	}
...
/**
Function used to set customized parameters during inquiry_scan state for G2
Sets desired scan_window size and interval
Girish
*/
static int hciops_set_inquiry_scan_params (int index)
{
	struct dev_info *dev = &devs[index];
	write_inq_activity_cp cp;
	cp.interval = 0x800; // 1.28 secs 
	cp.window = 0x12; // 11.25 ms DEFAULT VALUE

	DBG("hci%d hciops_set_inquiry_scan_params", index);

	if (hci_send_cmd(dev->sk, OGF_HOST_CTL, OCF_WRITE_INQ_ACTIVITY,
								sizeof(write_inq_activity_cp), &cp) < 0)
		return -errno;
...
static int hciops_set_discoverable(int index, gboolean discoverable)
{
	...
	if ( hciops_set_inquiry_scan_params(index) == -errno )
		DBG("hci could not set inquiry scan parameters");
...
static void set_event_mask(int index)
{
	...
	uint8_t cod_and_mask[] = {0x20, 0x04, 0x00, 0x00, 0x1E, 0x00}; 
    set_event_flt_cp cp;
	...
	/*Setting Inquiry Filter for Audio devices only */
    memset(&cp, 0, sizeof(cp));
    cp.flt_type = 0x01;
    cp.cond_type = 0x01;
    memcpy(cp.condition,cod_and_mask,6);
    hci_send_cmd(dev->sk, OGF_HOST_CTL, OCF_SET_EVENT_FLT,SET_EVENT_FLT_CP_SIZE, &cp);
...
static gboolean init_adapter(int index)
{
	...
	/* Setting default pairable to TRUE
	*  Because BT user interface has been
	*  removed for G2.
	*/
	pairable = TRUE;
...
static void create_ext_inquiry_response(int index, uint8_t *data)
{
	...
	/*Adding Manufacture specific Data*/
	{
	    uint16_t cmp = 0x0008; /* Motorola Comp Id */
		*ptr++ = 3 + sizeof(main_opts.manufacturespecific);
		*ptr++ = 0xFF;
		*ptr++ = (cmp & 0x00ff);
		*ptr++ = (cmp & 0xff00) >> 8;
        memcpy(ptr,main_opts.manufacturespecific,sizeof(main_opts.manufacturespecific));      
	}
...
static int write_class(int index, uint32_t class)
{
	...
	/*Hardcoding the Class of device: TODO: Need to check where is this API been triggered*/
    class = 0x280408;
...
static void write_class_complete(int index, uint8_t status)
{
	...
	{
		/*Commented By Sharan*/
		/*Reason:: Causing an infinite loop due to 
		which bluetoothd didn't go idle* 
		This is a temp fix need to check why it is ending here*/
...
static inline void remote_name_information(int index, void *ptr)
{
	...
	if(btd_QueryingForDeviceInRange(&evt->bdaddr))
	{
		DBG("btd_QueryingForDeviceInRange is true Calling btd_event_querry_remote_name");
		btd_event_querry_remote_name(&dev->bdaddr, &evt->bdaddr, evt->status);
	}
	else
	{
...
static inline void disconn_complete(int index, void *ptr)
{
	...
	btd_event_disconn_complete(&dev->bdaddr, &conn->bdaddr,evt->reason);//added disconnect reason
	conn_free(conn);
}
...
static void device_event(int event, int index, uint32_t data)
{
	switch (event) {
	case HCI_DEV_REG:
	...
	case SMP_PASSKEY_GEN:
		info("SMP Pass key %d", data);
		btd_event_le_passkey_notify(index, &data);
		break;
	}
}
...
static gboolean init_known_adapters(gpointer user_data)
{
	...
	for (i = 0; i < dl->dev_num; i++, dr++) {
		...
		device_event(HCI_DEV_UP, dr->dev_id, 0);
	}
								...
								gpointer data)
{
	...
	case EVT_SI_DEVICE:
		sd = (void *) &si->data;
		device_event(sd->event, sd->dev_id, sd->data);
		break;
	}
...
static int hciops_write_link_timeout(int index, bdaddr_t *bdaddr,
					uint32_t num_slots)
{
	struct dev_info *dev = &devs[index];
	uint16_t handle = 0;
	...
	ba2str(bdaddr, addr);
	DBG("hci%d addr %s which %d timeout %d", index, addr, num_slots);

	err = get_handle(index, bdaddr, &handle);
	if (err < 0)
		return err;

	if(hci_write_link_supervision_timeout(dev->sk, htobs(handle),
						htobs(num_slots), 1000) < 0)
		return -errno;
...
static struct btd_adapter_ops hci_ops = {
	...
	.write_link_timeout = hciops_write_link_timeout,
};
