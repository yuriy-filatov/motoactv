...
OMX_ERRORTYPE VIDDEC_Load_Defaults (VIDDEC_COMPONENT_PRIVATE* pComponentPrivate, OMX_S32 nPassing)
{
    ...
    switch(nPassing){
        ...
        case VIDDEC_INIT_VARS:
            ...
            pComponentPrivate->AVCFrameCounter					= 0;
...
OMX_ERRORTYPE VIDDEC_HandleCommandFlush(VIDDEC_COMPONENT_PRIVATE *pComponentPrivate, OMX_U32 nParam1, OMX_BOOL bPass)
{
    ...
    if ( nParam1 == VIDDEC_INPUT_PORT || nParam1 == OMX_ALL){
        ...
        if(bPass) {
            ...
            /*OMAPS00227444: Just applicable for H264 use cases.
            * If we have flushed and the frame count is less than MAX_PRIVATE_IN_BUFFERS
            * is a sign that the IB with SPS and PPS info is not being processed in SN. The CSR scenario
            * shows always frameCounter = 4 for error cases.
            * Here we are forcing to add SPS and PPS info to the first IB after the flush if the current
            * frame count is less than MAX_PRIVATE_IN_BUFFERS and the first header has been passed but not
            * sure if processed in DSP. */
            LOGE("AVCFrameCounter =%d frameCounter=%d", pComponentPrivate->AVCFrameCounter, pComponentPrivate->frameCounter);
            if(pComponentPrivate->pInPortDef->format.video.eCompressionFormat == OMX_VIDEO_CodingAVC &&
                pComponentPrivate->bFirstHeader && pComponentPrivate->AVCFrameCounter <= MAX_PRIVATE_IN_BUFFERS){
                LOGE("%d %s DSP flushed without processing SPS/PPS. saving first buffer\n", __LINE__, __func__);
            ...
            if ((pPortDefIn->bEnabled == OMX_TRUE && pPortDefOut->bEnabled == OMX_TRUE) ||
               ...
               break;
                ...
                if (pComponentPrivate->bDynamicConfigurationInProgress == OMX_TRUE ||
                    ...
                    pComponentPrivate->eLCMLState = VidDec_LCML_State_Start;
	                pComponentPrivate->bDynamicConfigurationInProgress = OMX_FALSE;
                    OMX_PRSTATE4(pComponentPrivate->dbg, "We were doing DynamicConfiguration, canceling it. %d \n",pComponentPrivate->bDynamicConfigurationInProgress);
                }
...
OMX_ERRORTYPE VIDDEC_HandleDataBuf_FromApp(VIDDEC_COMPONENT_PRIVATE *pComponentPrivate)
{
    ...
    if ((pComponentPrivate->eState == OMX_StateIdle) && (pComponentPrivate->eExecuteToIdle != OMX_StateExecuting)) {
        if (pBuffHead != NULL) {
    ...
    if(pBuffHead->nFlags & OMX_BUFFERFLAG_EOS){
        ...
        if(pBuffHead->nFilledLen != 0) { /*TODO: Validate this lagic*/
            if (pComponentPrivate->pInPortDef->format.video.eCompressionFormat == OMX_VIDEO_CodingAVC) {
                ...
                if ((pBuffHead->nFlags & OMX_BUFFERFLAG_EOS) == 0) {
                    ...
                    /*START OMAPS00227444 & OMAPS00231735 Related fix*/
                    if(pComponentPrivate->pInPortDef->format.video.eCompressionFormat == OMX_VIDEO_CodingAVC &&
                        pComponentPrivate->frameCounter >= pComponentPrivate->AVCFrameCounter){

                        pComponentPrivate->AVCFrameCounter = pComponentPrivate->frameCounter;
                    }
                    /*END OMAPS00227444 & OMAPS00231735 Related fix*/
		...
		if ((pBuffHead->nFlags & OMX_BUFFERFLAG_CODECCONFIG) == 0 ) {	//tag if not equal to OMX_BUFFERFLAG_CODECCONFIG
            ...
            else if ((pComponentPrivate->eLCMLState ==  VidDec_LCML_State_Start ) &&
