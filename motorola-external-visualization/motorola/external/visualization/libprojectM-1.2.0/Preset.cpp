...
void Preset::initialize(const std::string & pathname)
{
...
#else
	PROJECTM_LOG_DEBUG("[Preset] loading file pathname %s",pathname.c_str());
#endif
  }
  if ((retval = loadPresetFile(pathname)) < 0)
  {
	if (PRESET_DEBUG)
	{
		...
		PROJECTM_LOG_CRITICAL("Preset::initialize failed to load file %s",pathname.c_str());
		return;
#else
...
void Preset::initialize(std::istream & in)
{
  ...
  if ((retval = readIn(in)) < 0)
  {
	...
	{
#ifdef PROJECTM_ANDROID_DISABLE_EXCEPTION
		PROJECTM_LOG_CRITICAL("Preset::initialize failed to load from stream");
		return;
#else
...
#else
		PROJECTM_LOG_DEBUG("[Preset::readIn] no left bracket found...");
#endif
	}
	return PROJECTM_FAILURE;
  }
...
#else
	PROJECTM_LOG_DEBUG("[Preset::readIn] loading of preset name failed");
#endif
...
#else
		PROJECTM_LOG_DEBUG("loadPresetFile: loading of file %s",pathname.c_str());
#endif
    }
	return PROJECTM_ERROR;
  }
