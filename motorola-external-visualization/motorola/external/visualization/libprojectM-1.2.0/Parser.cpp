...
token_t Parser::parseToken(std::istream &  fs, char * string)
{
  ...
  for (i = 0; i < MAX_TOKEN_SIZE;i++)
  {
	...
	std::ostringstream buffer;
...
#else
			PROJECTM_LOG_DEBUG("Parser::parseToken token wrap! line %d",line_count);
#endif
        }
        while (c != '=')
        {
...
#else
				PROJECTM_LOG_DEBUG("Parser::parseToken token wrap: end of file ");
#endif
	        }
            return tEOF;
          }
		...
		{
#ifdef ENABLE_PRINTF
			std::cerr << "parseToken: parsed away equal sign, line prefix is \""  << buffer.str() 
		<< "\"" << std::endl;
#else
			PROJECTM_LOG_DEBUG("Parser::parseToken parsed away equal sign, line prefix is %s", buffer.str());
#endif
        }
        --i;
...
#else
		PROJECTM_LOG_DEBUG("Parser::parseToken token wrap around = false (LINE %d )",line_count);
#endif
      }
      return tSemiColon;
	...
	/// @bug remove this nonsense
        if (c == '\r')
        {
          ...
          abort();
#else
		  PROJECTM_LOG_DEBUG("Parser::parseToken bug R");
#endif
        }
        if (c == '\b')
        {
          ...
          abort();
#else
		  PROJECTM_LOG_DEBUG("Parser::parseToken bug B");
#endif
        }
        string[i] = tolower(c);
        //string[i+1] = 0;
...
int Parser::parse_line(std::istream &  fs, Preset * preset)
{
  ...
  case tDiv:
	...
	{
#ifdef ENABLE_PRINTF
		std::cerr << "parse_line: invalid token found at start of line (LINE "
      << line_count << ")" << std::endl;
#else
		PROJECTM_LOG_DEBUG("Parser::parse_line: invalid token found at start of line (LINE %d )",line_count);
#endif
    }
  ...
  case tEq:
	...
	{
#ifdef ENABLE_PRINTF
		std::cout << "last line prefix = \"" << eqn_string << "\"" << std::endl;
#else
		PROJECTM_LOG_DEBUG("Parser::parse_line last line prefix = %s ",eqn_string);
#endif
    }
...
#ifdef PROJECTM_ANDROID
    printf("parse_line: PROJECTM_ANDROID , token val = %d EOF passed\n", token);
    line_mode = UNSET_LINE_MODE;
    line_count = 1;
    tokenWrapAroundEnabled = false;
    return EOF;
    ...
    if ((func = BuiltinFuncs::find_func(string)) != NULL)
    {
...
#else
		PROJECTM_LOG_DEBUG("Parser::parse_gen_expr  found prefix function LINE %d",line_count);
#endif
      }
      ...
      if ((expr_list = parse_prefix_args(fs, func->getNumArgs(), preset)) == NULL)
      {
        if (PARSE_DEBUG)
        {
          ...
          << line_count << ")" << std::endl;
#else		
		   PROJECTM_LOG_DEBUG("Parser::parse_gen_expr failed to generate an expresion list! (LINE %d",line_count);
#endif
        }
        if ( tree_expr != NULL )
        {
...
#ifdef PROJECTM_ANDROID
	//if (PARSE_DEBUG) 
      		//PROJECTM_LOG_DEBUG("Parser::parseToken error token %d\n",token);
	//
      //line_count = 1;
      //line_mode = UNSET_LINE_MODE;
      //return tEOF;
  ...
  if ((param = ParamUtils::find<ParamUtils::NO_CREATE>(var_string, &custom_shape->text_properties_tree)) != NULL)
  {
    ...
    *((std::string*)param->engine_val) = text;
    if (PARSE_DEBUG)
    {
      ...
      << text << "\"" << std::endl;
#else
	  PROJECTM_LOG_DEBUG("parse_shapecode: found image url, text is %s",text.c_str());
#endif
    }
    return PROJECTM_SUCCESS;
  }
