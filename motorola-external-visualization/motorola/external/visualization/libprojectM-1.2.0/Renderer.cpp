...
static void check_for_GL_errors( const char *calling_location )
{
	...
	{
		PROJECTM_LOG_DEBUG( "OpenGL Error @ %s: %i", calling_location, err_code );
		err_code = glGetError();
	}
}
#else
#define check_for_GL_errors(x) //comment
#endif

#ifdef PROJECTM_ANDROID
static void glMotRectf (GLfloat x1, GLfloat y1, GLfloat x2, GLfloat y2)
{
	float points[4][2];
	points[0][0] = x1;
	points[0][1] = y1;
	...
	check_for_GL_errors("glMotRectf glVertexPointer");
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4); 
	check_for_GL_errors("glMotRectf glDrawArrays");
}
#endif
...
{
    PROJECTM_LOG_DEBUG("Renderer::stopRenderer");
...
{
        PROJECTM_LOG_DEBUG("Renderer::ResetTextures");
	textureManager->Clear();
...
{
	//PROJECTM_LOG_DEBUG("Renderer::RenderFrame enter presetOutputs %p presetInputs %p",presetOutputs,presetInputs);
	/** Save original view state */
	check_for_GL_errors("RenderFrame init");
	...
	check_for_GL_errors("RenderFrame Before PASS1");
	//Lock FBO
	renderTarget->lock();
	
	glViewport( 0, 0, renderTarget->texsize, renderTarget->texsize );
	//PROJECTM_LOG_DEBUG("Renderer::RenderFrame renderTarget->texsize %d ",renderTarget->texsize);
	...
	check_for_GL_errors("RenderFrame Before Interpolation");
	if(this->renderTarget->useFBO)
	{
	...
	check_for_GL_errors("RenderFrame after Interpolation");
	//    if(!this->renderTarget->useFBO)
	{
		draw_motion_vectors(presetOutputs);
	}
	check_for_GL_errors("RenderFrame after draw_motion_vectors");
	
	draw_shapes(presetOutputs);
	check_for_GL_errors("RenderFrame after draw_shapes");
	draw_custom_waves(presetOutputs);
	check_for_GL_errors("RenderFrame after draw_custom_waves");
	draw_waveform(presetOutputs);
	check_for_GL_errors("RenderFrame after draw_waveform");
	if(presetOutputs->bDarkenCenter)darken_center();
	draw_borders(presetOutputs);
	check_for_GL_errors("RenderFrame after draw_borders");
	draw_title_to_texture();
	check_for_GL_errors("RenderFrame after draw_title_to_texture");
	...
	check_for_GL_errors("RenderFrame after restore view state");
	renderTarget->unlock();
	PROJECTM_LOG_DEBUG("Renderer::RenderFrame PASS1 over");
	check_for_GL_errors("RenderFrame pass1 over");
...
#endif
	PROJECTM_LOG_DEBUG("Renderer::RenderFrame glViewport %d x %d", this->vw, this->vh);
	glViewport( 0, 0, this->vw, this->vh );
	
	
	PROJECTM_LOG_DEBUG("Renderer::RenderFrame render texture %d", this->renderTarget->textureID[0]);
	glBindTexture( GL_TEXTURE_2D, this->renderTarget->textureID[0] );
	
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame GL_PROJECTION");
	glMatrixMode(GL_PROJECTION);
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame glLoadIdentity");
	glLoadIdentity();
#ifdef USE_GLES1
...
#endif
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame glBlendFunc");
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame glLineWidth");
	...
	/*if(this->studio%2)render_texture_to_studio(presetOutputs, presetInputs);
        ...
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame render_texture_to_screen");
        render_texture_to_screen(presetOutputs);
	
	//PROJECTM_LOG_DEBUG("Renderer::RenderFrame studio %d",studio);
	check_for_GL_errors("RenderFrame after render_texture_to_screen");
	
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame glMatrixMode");
	glMatrixMode(GL_MODELVIEW);
	glTranslatef(-0.5, -0.5, 0);

#ifndef PROJECTM_ANDROID	
	// When console refreshes, there is a chance the preset has been changed by the user
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame refreshConsole");
	refreshConsole();
	...
	glTranslatef(0.5 , 0.5, 0);
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame refreshConsole end");
#endif

#ifdef USE_FBO
	PROJECTM_LOG_DEBUG("Renderer::RenderFrame FBO");
	if(renderTarget->renderToTexture)
		glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
#endif
	check_for_GL_errors("RenderFrame Exit");
	//PROJECTM_LOG_DEBUG("Renderer::RenderFrame exit");
}
...
Renderer::~Renderer()
{
	...
	if (renderTarget) {
                PROJECTM_LOG_DEBUG("Renderer::~Renderer delete renderTarget");
		delete(renderTarget);
        }
	if (textureManager)
...
void Renderer::reset(int w, int h)
{
	...
	//BEGIN Moto, fbwm38, 29-Sep-2010, IKSTABLETWOV-231
	//Added NULL check for renderTarget to avoid crash
	if(this->renderTarget != NULL)
	{
        ...
        {
		PROJECTM_LOG_DEBUG("Renderer::reset RendererTarget is NULL");
        }
	//END Moto, IKSTABLETWOV-231
}
...
void Renderer::draw_shapes(PresetOutputs *presetOutputs)
{
	...
	//PROJECTM_LOG_DEBUG ("Renderer::draw_shapes size %d",presetOutputs->customShapes.size());
	...
	pos != presetOutputs->customShapes.end(); ++pos)
	{
		...
		if( (*pos)->enabled==1)
		{
			//PROJECTM_LOG_DEBUG ("Renderer::draw_shapes enabled ");
...
#ifdef PROJECTM_ANDROID
	glMotRectf(of, of, of+iff, texof);
	glMotRectf(of+iff, of, texof-iff, of+iff);
	glMotRectf(texof-iff, of, texof, texof);
	glMotRectf(of+iff, texof, texof-iff, texof-iff);
#else
...
void Renderer::render_texture_to_screen(PresetOutputs *presetOutputs)
{
	...
	if (presetOutputs->bInvert)
	{
        ...
        PROJECTM_LOG_DEBUG("Renderer::RenderFrame render_texture_to_screen Exit");
}
