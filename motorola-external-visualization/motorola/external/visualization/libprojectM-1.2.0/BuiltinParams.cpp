...
#ifdef PROJECTM_ANDROID_DISABLE_EXCEPTION
	PROJECTM_LOG_CRITICAL("BuiltinParams::BuiltinParams failed to allocate builtin parameter database with error");
#else
	throw ret;
#endif
  }
    ...
    float init_val, float upper_bound, float lower_bound, const std::string & alt_name)
{
  ...
  if (BUILTIN_PARAMS_DEBUG == 2)
  {
    ...
    fflush(stdout);
#else
	PROJECTM_LOG_DEBUG("load_builtin_param_float: (name \"%s\") (alt_name = \"%s\") ", name.c_str(), alt_name.c_str());
#endif
  }
...
std::transform(lowerName.begin(), lowerName.end(), lowerName.begin(), tolower);
  ...
  if (BUILTIN_PARAMS_DEBUG == 2)
  {
    ...
    fflush(stdout);
#else
	PROJECTM_LOG_DEBUG("BuiltinParams::load_builtin_param_float created...");
#endif
  }
  ...
  if (BUILTIN_PARAMS_DEBUG == 2)
  {
    ...
    fflush(stdout);
#else
	PROJECTM_LOG_DEBUG("BuiltinParams::load_builtin_param_float inserted...");
#endif
  }
  ...
  if (alt_name != "")
  {
    ...
    if (BUILTIN_PARAMS_DEBUG == 2)
    {
      ...
      fflush(stdout);
#else
		PROJECTM_LOG_DEBUG("BuiltinParams::load_builtin_param_float alt_name inserted..");
#endif
    }
  ...
  if (BUILTIN_PARAMS_DEBUG == 2)
  {
  	...
  	printf("finished\n");
#else
	PROJECTM_LOG_DEBUG("BuiltinParams::load_builtin_param_float finished");
#endif
  }
...
int BuiltinParams::init_builtin_param_db(const PresetInputs & presetInputs, PresetOutputs & presetOutputs)
{
  ...
  if (BUILTIN_PARAMS_DEBUG)
  {
    ...
    fflush(stdout);
#else
	PROJECTM_LOG_DEBUG("BuiltinParams::load_builtin_param_bool init_builtin_param: loading database...");
#endif
  }
...
#else
		PROJECTM_LOG_DEBUG("BuiltinParams::load_builtin_param_bool failed loading builtin parameters (FATAL)");
#endif
    }
	return PROJECTM_ERROR;
  ...
  if (BUILTIN_PARAMS_DEBUG)
  {
  	...
  	printf("success!\n");
#else
	PROJECTM_LOG_DEBUG("BuiltinParams::load_builtin_param_bool success!");
#endif
  }
