...
RenderTarget::~RenderTarget() {
    ...
    PROJECTM_LOG_DEBUG("RenderTarget::~RenderTarget delete texture %d",textureID[0]);
...
RenderTarget::RenderTarget(int texsize, int width, int height) : useFBO(false) {
   ...
   this->useFBO=false;
    ...
    PROJECTM_LOG_DEBUG("original width x height %d x %d, new texsize %d", width, height, this->texsize);
  

 
        glGenTextures(1, &this->textureID[0] );
	PROJECTM_LOG_DEBUG("RenderTarget::RenderTarget glGenTextures %d",textureID[0] );
	// check_for_GL_errors("RenderTarget::RenderTarget glGenTextures");
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	check_for_GL_errors("RenderTarget::RenderTarget glTexParameterf");
        ...
        glTexImage2D(GL_TEXTURE_2D,
		    ...
		    0,
		    /*GL_RGBA ProjectM_Android*/GL_RGB, 
		    GL_UNSIGNED_BYTE,
		    NULL);
	check_for_GL_errors("RenderTarget::RenderTarget glTexImage2D");
  ...
  {
  	PROJECTM_LOG_DEBUG("RenderTarget::fallbackRescale ");
#if 0
...
void RenderTarget::unlock() {
  ...
  if(this->useFBO)
    ...
    glBindTexture( GL_TEXTURE_2D, this->textureID[0] );
    check_for_GL_errors("RenderTarget::unlock glBindTexture");
    PROJECTM_LOG_DEBUG("RenderTarget::unlock() glBindTexture %d",textureID[0]);
    
    PROJECTM_LOG_DEBUG("RenderTarget::unlock() glCopyTexSubImage2D %d",this->texsize);

	glCopyTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, 0, 0, this->texsize, this->texsize );
//    check_for_GL_errors("RenderTarget::unlock() after glCopyTexSubImage2D ");
	GLenum err_code = glGetError();
	if (err_code != GL_NO_ERROR)
	{
		PROJECTM_LOG_DEBUG("RenderTarget::unlock() ERROR %i ", err_code);
		
 	        glDeleteTextures( 1, &this->textureID[0]);

		glGenTextures(1, &this->textureID[0] );
		PROJECTM_LOG_DEBUG("RenderTarget::unlock glGenTextures %d",textureID[0] );
		check_for_GL_errors("RenderTarget::unlock glGenTextures");
		...
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		check_for_GL_errors("RenderTarget::unlock glTexParameterf");
		...
		glTexImage2D(GL_TEXTURE_2D,
					    ...
					    GL_UNSIGNED_BYTE,
					    NULL);
		check_for_GL_errors("RenderTarget::unlock() after glTexImage2D ");
	}
  }
