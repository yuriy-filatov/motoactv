...
public:
  ...
  DLLEXPORT projectM(std::string config_file, int aWidth, int aHeight, int flags = FLAG_NONE);
  ...
  DLLEXPORT void stopRender();
  DLLEXPORT void resumeRender();
...
private:
  ...
  void readConfig(const std::string & configFile, int aWidth, int aHeight);
