...
#define PROJECTM_RES_PATH "/data/data/"
...
DLLEXPORT  projectM::projectM ( std::string config_file, int aWidth, int aHeight, int flags) :
		beatDetect ( 0 ), renderer ( 0 ),  _pcm(0), m_presetPos(0), m_flags(flags)
{
	readConfig ( config_file, aWidth, aHeight );	
	projectM_reset();
...
void projectM::readConfig (const std::string & configFile, int aWidth, int aHeight )
{
#ifdef ENABLE_PRINTF
	std::cout << "configFile: " << configFile << std::endl;
#else
	PROJECTM_LOG_DEBUG("projectM::readConfig %s",configFile.c_str());
#endif
	ConfigFile config ( configFile );
...
#ifdef PROJECTM_ANDROID
	_settings.windowWidth  = aWidth;
	_settings.windowHeight = aHeight;
	PROJECTM_LOG_DEBUG("projectM::readConfig _settings.windowWidth %d _settings.windowHeight %d",_settings.windowWidth,_settings.windowHeight);
#else
	...
	char path[256];
	path[0] = '\0';
	strcat(path, PROJECTM_RES_PATH);
	strcat(path,"presets");
	_settings.presetURL = config.read<string> ( "Preset Path", path );
	#else
	...
	path[0] = '\0';
	strcat(path, PROJECTM_RES_PATH);
	strcat(path,"fonts/VeraMono.ttf");
	_settings.menuFontURL = config.read<string> 
			( "Menu Font", path );	
	#else
	_settings.titleFontURL = config.read<string> 
...
void projectM::setupPresetInputs(PresetInputs *inputs)
{
  ...
  inputs->treb_att = beatDetect->treb_att;
	PROJECTM_LOG_DEBUG("projectM::setupPresetInputs exit");
}
...
DLLEXPORT void projectM::renderFrame()
{
	...
	PROJECTM_LOG_DEBUG("projectM::renderFrame mspf %d",mspf);
	...
	PROJECTM_LOG_DEBUG("projectM::renderFrame got frame");
	...
	{
		PROJECTM_LOG_DEBUG("projectM::renderFrame case 1");
		if ( timeKeeper->PresetProgressA()>=1.0 && !timeKeeper->IsSmoothing())
		{
			PROJECTM_LOG_DEBUG("projectM::renderFrame case 11");
 			
			timeKeeper->StartSmoothing();		      
			PROJECTM_LOG_DEBUG("projectM::renderFrame case startSmoothing");
			//	printf("Start Smooth\n");
			...
			PROJECTM_LOG_DEBUG("projectM::renderFrame switchPreset ");
			presetSwitchedEvent(false, **m_presetPos);
			PROJECTM_LOG_DEBUG("projectM::renderFrame presetSwitchedEvent");
		}
		
		else if ( ( beatDetect->vol-beatDetect->vol_old>beatDetect->beat_sensitivity ) && timeKeeper->CanHardCut() )
		{
			PROJECTM_LOG_DEBUG("projectM::renderFrame case 12");
		  // printf("Hard Cut\n");
			switchPreset(m_activePreset, presetInputs, presetOutputs);
			PROJECTM_LOG_DEBUG("projectM::renderFrame switchPreset ");

			timeKeeper->StartPreset();
			PROJECTM_LOG_DEBUG("projectM::renderFrame StartPreset");
	...
	{
		PROJECTM_LOG_DEBUG("projectM::renderFrame case 2");
	  ...
	  //	 printf("start thread\n");
...
#ifdef USE_THREADS
                PROJECTM_LOG_DEBUG("projectM::renderFrame signal");
		pthread_cond_signal(&condition);
		pthread_mutex_unlock( &mutex );
#endif
                PROJECTM_LOG_DEBUG("projectM::renderFrame evaluateFrame");
		m_activePreset->evaluateFrame();
                PROJECTM_LOG_DEBUG("projectM::renderFrame PerPixelMath");
		renderer->PerPixelMath ( &m_activePreset->presetOutputs(), &presetInputs );
                PROJECTM_LOG_DEBUG("projectM::renderFrame WaveformMath");
		renderer->WaveformMath ( &m_activePreset->presetOutputs(), &presetInputs, true );

#ifdef USE_THREADS
                PROJECTM_LOG_DEBUG("projectM::renderFrame pthread_mutex_lock lock");
		pthread_mutex_lock( &mutex );
#else		
                PROJECTM_LOG_DEBUG("projectM::renderFrame evaluateSecondPreset");
		evaluateSecondPreset();
#endif
		
		PROJECTM_LOG_DEBUG("projectM::renderFrame case 2 before merge presets");
		...
		PROJECTM_LOG_DEBUG("projectM::renderFrame case 2 after merge presets");
	}
	else
	{
		PROJECTM_LOG_DEBUG("projectM::renderFrame case 3");
		if ( timeKeeper->IsSmoothing() && timeKeeper->SmoothRatio() > 1.0 )
		{
			PROJECTM_LOG_DEBUG("projectM::renderFrame case 31");
		  //printf("End Smooth\n");
		...
		PROJECTM_LOG_DEBUG("projectM::renderFrame before evaluateFrame");
		m_activePreset->evaluateFrame();
		PROJECTM_LOG_DEBUG("projectM::renderFrame after evaluateFrame");

		PROJECTM_LOG_DEBUG("projectM::renderFrame before >PerPixelMath");
		renderer->PerPixelMath ( &m_activePreset->presetOutputs(), &presetInputs );
		PROJECTM_LOG_DEBUG("projectM::renderFrame before WaveformMath");
	...
	//	renderer->presetName = m_activePreset->absoluteFilePath();
		...
		PROJECTM_LOG_DEBUG("projectM::renderFrame RenderFrame");
...
DLLEXPORT void projectM::stopRender()
{
...
DLLEXPORT void projectM::resumeRender()
{
     if(this->renderer != NULL) {
...
#else
		PROJECTM_LOG_CRITICAL("[projectM] error allocating preset loader");
#endif
...
#else
		PROJECTM_LOG_CRITICAL("[projectM] error allocating preset chooser");
#endif
		return PROJECTM_FAILURE;
	}
