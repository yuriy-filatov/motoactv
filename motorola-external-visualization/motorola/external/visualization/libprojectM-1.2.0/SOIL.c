...
{
	PROJECTM_LOG_DEBUG("SOIL_internal_create_OGL_texture ");
	/*	variables	*/
    ...
    tex_id = reuse_texture_ID;
    #if SOIL_CHECK_FOR_GL_ERRORS
	check_for_GL_errors( "glGenTextures1" );
	PROJECTM_LOG_DEBUG("SOIL_internal_create_OGL_texture tex_id %d",tex_id);
	#endif
    if( tex_id == 0 )
    {
	...
	if( tex_id )
	{
		...
		PROJECTM_LOG_DEBUG("SOIL_internal_create_OGL_texture channels %d",channels);
		original_texture_format = GL_RGB;
		...
		if( DXT_mode == SOIL_DXT_DIRECT_UPLOAD )
		{
			...
			} else
			{
				...
				#if SOIL_CHECK_FOR_GL_ERRORS
				check_for_GL_errors( "glTexImage2D SOIL direct upload" );
				#endif
				/*	printf( "OpenGL DXT compressor\n" );	*/
			}
		} else
		{
			PROJECTM_LOG_DEBUG("opengl_texture_target %d internal_texture_format %d",opengl_texture_target,internal_texture_format);
			PROJECTM_LOG_DEBUG("width %d height %d original_texture_format %d",width,height,original_texture_format);
			...
			glTexImage2D(
				...
				original_texture_format, GL_UNSIGNED_BYTE, img );
			#if SOIL_CHECK_FOR_GL_ERRORS
			check_for_GL_errors( "glTexImage2D SOIL test1" );
			#endif
			/*printf( "OpenGL DXT compressor\n" );	*/
		}
			...
			while( ((1<<MIPlevel) <= width) || ((1<<MIPlevel) <= height) )
			{
				...
				} else
				{
					...
					glTexImage2D(
						...
						original_texture_format, GL_UNSIGNED_BYTE, resampled );
					#if SOIL_CHECK_FOR_GL_ERRORS
					check_for_GL_errors( "glTexImage2D internal OGL texture" );
					#endif
				}
	...
	)
{
	unsigned char *result = stbi_load_from_memory(
				...
				width, height, channels,
				force_channels );
	PROJECTM_LOG_DEBUG("SOIL_load_image_from_memory result %p",result);
	if( result == NULL )
	{
		result_string_pointer = stbi_failure_reason();
		PROJECTM_LOG_DEBUG("SOIL_load_image_from_memory failed %s",result_string_pointer);
	} else
	{
...
int query_DXT_capability( void )
{
	...
	if( has_DXT_capability == SOIL_DXT_UNKNOWN )
	{
		...
		} else
		{
			...
			#else
			#ifdef PROJECTM_ANDROID
				PROJECTM_LOG_DEBUG("SOIL query_DXT_capability glCompressedTexImage2D %p",glCompressedTexImage2D);
