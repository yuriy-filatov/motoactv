...
typedef WTF::HashMap<UChar, const char*> EntityMapType;
...
const EntityMapType* EntityMapData::GetEntityMapData() {
  if (!map_) {
    ...
    for (int i = 0; i < entity_codes_length_; ++i, ++entity_code) {
      ...
      EntityMapType::iterator it = map_->find(entity_code->code);
      if (it != map_->end() &&
          strcmp(entity_code->name, it->second) == 0)
        continue;
      if (!standard_html_entities_ ||
		...
		map_->set(entity_code->code, entity_code->name);
    }
    if (standard_html_entities_)
      //(*map_)[0x0027] = "#39";
	  map_->set(0x0027, "#39");
...
const char* EntityMap::GetEntityNameByCode(UChar code, bool is_html) {
