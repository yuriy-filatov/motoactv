...
// BEGIN Motorola, E11787, 2009/11/19, IKMAP-2542 / SavePage, Context Menu Enhancement, Browser Shortcut
#ifdef ANDROID_MOTO_SAVEPAGE
            case DocCSSs:  //all css elements with a value for href
                if (e->hasLocalName(linkTag) && (!e->getAttribute(hrefAttr).isNull()))
                    return e;
                break;
            case DocMiscRes:  //all resource elements with a value for href for savepage
                if (e->hasLocalName(bodyTag) && (!e->getAttribute(backgroundAttr).isNull()))
                    return e;
                if (e->hasLocalName(inputTag) && (!e->getAttribute(srcAttr).isNull()))
                    return e;
                break;
#endif
// END IKMAP-2542
