...
/* Playback */
	...
	/* use mutex so that the following call to dapm_sync() does not
	   interfere with a set_bias_level() call already in progress */
	mutex_lock(&cs42l73_codec->mutex);
        snd_soc_dapm_sync(codec);
	mutex_unlock(&cs42l73_codec->mutex);
}
