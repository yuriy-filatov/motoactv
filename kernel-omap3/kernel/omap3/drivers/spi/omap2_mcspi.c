...
#define OMAP2_MCSPI_FIFOENABLE       1
					...
					int enable)
{
	...
	if (enable == 1) {
		...
		if (buf_size < (mcspi->fifo_depth >> 2))
...
static void endian_convert_buffer(u8 *buf, int len)
{
	int i = 0;
	u32 data;
	void *pbuf = &buf[0];

	/* Since we are doing 32 bit SPI writes convert the buffer from
	 * little endian to big endian except for the last byte which is sent
	 * as single byte transfer */
...
omap2_mcspi_txrx_dma(struct spi_device *spi, struct spi_transfer *xfer)
{
	...
	if (tx != NULL) {
		...
		if (mcspi->fifo_depth != 0) {
			/* enable 32 bit word length */
			l = mcspi_cached_chconf0(spi);
			l &= ~OMAP2_MCSPI_CHCONF_WL_MASK;
			l |= (32 - 1) << 7;
			mcspi_write_chconf0(spi, l);
			omap2_mcspi_set_txfifo(spi, (count-(count%64)) >> 2, 1);
		}
	}
	...
	if (tx != NULL) {
		...
		if (mcspi->fifo_depth != 0) {
			...
			/* enable original word length */
			l = mcspi_cached_chconf0(spi);
			l &= ~OMAP2_MCSPI_CHCONF_WL_MASK;
			l |= (cs->word_len - 1) << 7;
			mcspi_write_chconf0(spi, l);

			/* flush the extra bytes below fifo depth */
			omap2_mcspi_set_enable(spi, 1);
			extra_bytes_cnt = count % mcspi->fifo_depth;
			while (extra_bytes_cnt) {
				extra_bytes_data =
					tx[xfer->len - extra_bytes_cnt];
				__raw_writel(extra_bytes_data,
					cs->base + OMAP2_MCSPI_TX0);
				udelay(100);
				extra_bytes_cnt--;
			}
			omap2_mcspi_set_enable(spi, 0);
...
static int omap2_mcspi_transfer(struct spi_device *spi, struct spi_message *m)
{
	...
	list_for_each_entry(t, &m->transfers, transfer_list) {
		...
		if (tx_buf != NULL) {
			...
			mcspi = spi_master_get_devdata(spi->master);
			if (mcspi->fifo_depth != 0)
				endian_convert_buffer((u8 *) tx_buf, len);
...
static int __init omap2_mcspi_probe(struct platform_device *pdev)
{
	...
	mcspi = spi_master_get_devdata(master);
	mcspi->master = master;
#if OMAP2_MCSPI_FIFOENABLE
	mcspi->fifo_depth = 64;
#else
	mcspi->fifo_depth = 0;
#endif
	mcspi->force_cs_mode = pdata->force_cs_mode;
	dev_info(&pdev->dev, "Configuring force_cs_mode to %d\n",
		 mcspi->force_cs_mode);
