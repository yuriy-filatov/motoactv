...
#define MAX_SLEEP_BUFFER_SIZE   10
char g_debug;
...
struct msp430_sleep_data {
	unsigned short epochcount;
	unsigned int timestamp;
};
...
struct msp430_sleep_data sdata_buffer[MAX_SLEEP_BUFFER_SIZE] = {};
...
void switch_msp430_mode(enum msp_mode mode)
{
	...
	/* bootloader mode */
	if (mode == BOOTMODE) {
		printk(KERN_INFO "MSP430 toggling to switch to boot mode\n");
	...
	/*normal mode */
		printk(KERN_INFO "MSP430 toggling to normal or factory mode\n");
		gpio_set_value(msp430_misc_data->pdata->gpio_test,
			!(test_pin_active_value));
		msleep_interruptible(I2C_RETRY_DELAY);
		gpio_set_value(msp430_misc_data->pdata->gpio_reset, 0);
			...
			int writelen, int readlen)
{
	...
	if (err != 2) {
		...
		if (ps_msp430->mode == NORMALMODE) {
			printk(KERN_INFO "Resetting msp due to i2c failures\n");
			/* reset msp */
...
static bool check_msp_reset(struct msp430_data *ps_msp430)
{
	bool ret = false;
	unsigned char buff[3];

	/* read msp reset detect byte */
	buff[0] = MSP_RESET_DETECT;
	if (msp430_i2c_write_read(ps_msp430, buff, 1, 1) < 0) {
		printk(KERN_ERR "Unable to read MSP_RESET_DETECT \n");
		return ret;
	}
	printk(KERN_INFO "MSP_RESET_DETECT read = 0x%x\n", buff[0]);
	if (buff[0] == 0x01)
		ret = true;
	/* clear msp reset detect byte */
	buff[0] = MSP_RESET_DETECT;
	buff[1] = 0x00;
	if (msp430_i2c_write(ps_msp430, buff, 2) < 0)
		printk(KERN_ERR "Unable to clear MSP_RESET_DETECT \n");
	return ret;
}
				...
				struct msp430_pedometer_data *curr_data)
{
	...
	/* check if msp reset */
	if (curr_data->stepcount < ps_msp430->prev_data.stepcount ||
	curr_data->distance < ps_msp430->prev_data.distance ||
	curr_data->activity < ps_msp430->prev_data.activity) {
		printk(KERN_INFO "Error !!! curr_step = %u, prev_step = %u,\
		curr_dist = %u, prev_dist = %u, curr_mets = %u,\
		prev_mets = %u\n", curr_data->stepcount,
		ps_msp430->prev_data.stepcount,	curr_data->distance,
		ps_msp430->prev_data.distance, curr_data->activity,
		ps_msp430->prev_data.activity);
		if (check_msp_reset(ps_msp430)) {
			ps_msp430->prev_data.stepcount = 0;
			ps_msp430->prev_data.distance = 0;
			ps_msp430->prev_data.activity = 0;
		}
	}
	...
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: MSP_LATITUDE\n");
		return;
	}
	...
	err = msp430_i2c_write_read(ps_msp430, buff, 1, 4);
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: MSP_LONGITUDE\n");
		return;
	}
	...
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: WALK_DIRN_QUALITY\n");
		return;
	}
	accuracy =  buff[0];
	...
	input_sync(ps_msp430->input_dev);
	KDEBUG(" Sending locdata : lat = %d, lon = %d, acc = %d",
		latitude, longitude, (int)accuracy);
}
		...
		struct msp430_passive_data *passive_data)
{
	...
	buffer_count++) {
		...
		if (err < 0) {
			printk(KERN_INFO "Reading from msp failed: MONITORME_60SEC_METS\n");
			return;
		}
...
static int msp430_read_sleep_data(struct msp430_data *ps_msp430,
	struct msp430_sleep_data *sleep_data)
{
	int err;
	int buffer_count;
	unsigned char buff[10];

	for (buffer_count = 0; buffer_count < MAX_SLEEP_BUFFER_SIZE ; \
	buffer_count++) {
		buff[0] = SLEEP_DATA;
		err = msp430_i2c_write_read(ps_msp430, buff, 1, 6);
		if (err < 0) {
			printk(KERN_INFO "Reading from msp failed: SLEEP_DATA\n");
			return err;
		}
		sdata_buffer[buffer_count].epochcount = ((buff[0] << 8) \
				|  buff[1]);
		sdata_buffer[buffer_count].timestamp = (buff[2] << 24) \
		| (buff[3] << 16) | (buff[4] << 8) | \
		(buff[5]);

		printk(KERN_INFO "Buffer - %d : Epochcount = %d, \
		Timestamp = %d\n", buffer_count, \
		sdata_buffer[buffer_count].epochcount,
		sdata_buffer[buffer_count].timestamp);

		if (sdata_buffer[buffer_count].timestamp != 0)
			sdata_buffer[buffer_count].timestamp = \
			sdata_buffer[buffer_count].timestamp - \
			NEW_EPOCH_DIFFERENCE;
	}
	return err;
}
...
static void msp430_report_sleep_values(struct msp430_data *ps_msp430,
	struct msp430_sleep_data *sleep_data)
{
	int i;
	for (i = 0; i < MAX_SLEEP_BUFFER_SIZE; i++) {
		if ((sdata_buffer[i].timestamp != 0) &&
				 (sdata_buffer[i].epochcount != 0)) {
			input_report_rel(ps_msp430->input_dev, REL_RZ, \
				sdata_buffer[i].epochcount);
			input_report_rel(ps_msp430->input_dev, \
				REL_MISC, sdata_buffer[i].timestamp);
			input_sync(ps_msp430->input_dev);
		}
	}
}

static int msp430_read_motion_data(struct msp430_data *ps_msp430,
		struct msp430_pedometer_data *pedo)
{
	...
	err = msp430_i2c_write_read(ps_msp430, buff, 1, 4);
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: MONITORME_10SEC_METS\n");
		return err;
	}
	...
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: TOTAL_STEP_COUNT\n");
		return err;
	}
	...
	err = msp430_i2c_write_read(ps_msp430, buff, 1, 2);
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: SPEED\n");
		return err;
	}
	...
	err = msp430_i2c_write_read(ps_msp430, buff, 1, 4);
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: TOTAL_DISTANCE\n");
		return err;
	}
...
static void msp430_kick_idletimer(struct msp430_data *ps_msp430){
	...
	printk(KERN_INFO "%s() set regsiter 0x%x to 0x%x\n", __func__,
			  buff[0], buff[1]);
...
void msp430_disable_offmode(void){
	...
	if (msp430_misc_data) {
		mutex_lock(&msp430_misc_data->lock);
...
static void msp430_irq_work_func(struct work_struct *work)
{
	...
	struct msp430_passive_data passive_data;
	unsigned char buff[10];
	enum msp_activities gesture_type;
	struct msp430_sleep_data sleep_data;
	...
	err = msp430_i2c_write_read(ps_msp430, buff, 1, 1);
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: INTERRUPT_STATUS\n");
		goto EXIT;
	}
	irq_status = buff[0];
	...
	err = msp430_i2c_write_read(ps_msp430, buff, 1, 1);
	if (err < 0) {
		printk(KERN_INFO "Reading from msp failed: INTERRUPT_MASK\n");
		goto EXIT;
	}
#ifdef G1
	if (irq_status & M_ACTIVITY_CHANGE) {
		if (msp430_read_motion_data(ps_msp430, &pedo) >= 0)
	...
	if (irq_status & M_ACTIVITY_CHANGE) {
		...
		if (err < 0) {
			printk(KERN_INFO "Reading from msp failed: \
					activity_change irq %s()\n", __func__);
			goto EXIT;
		}
		/* Add 1 to allow still(0) to pass up a REL event */
		printk(KERN_INFO "Got act_change act=%d\n", buff[0]+1);
	...
	if (irq_status & M_MME_60) {
		printk(KERN_INFO "detected PASSIVE_MODE\n");
	...
	if (irq_status & M_TAP_TAP) {
		KDEBUG(KERN_INFO "Got gesture interrupt\n");
		buff[0] = GESTURE_TYPE;
		err = msp430_i2c_write_read(ps_msp430, buff, 1, 1);
		if (err < 0) {
			printk(KERN_INFO "Reading from msp failed: \
		...
		if (buff[0] == GESTURE_TYPE_TAP) {
			gesture_type = TAP;
			KDEBUG(KERN_ALERT "Got tap interrupt\n");
			/* Add 1 to allow still(0) to pass up a REL event */
			input_report_rel(ps_msp430->input_dev,
				REL_DIAL, gesture_type+1);
			input_sync(ps_msp430->input_dev);
		} else if (buff[0] == GESTURE_TYPE_SCREEN_ON) {
			gesture_type = SCREEN_ON;
			KDEBUG(KERN_ALERT "Got screen on interrupt\n");
			/* Add 1 to allow still(0) to pass up a REL event */
			input_report_rel(ps_msp430->input_dev,
				REL_DIAL, gesture_type+1);
			input_sync(ps_msp430->input_dev);
	...
	if (irq_status & M_CALIBRATION_COMPLETE_WALK) {
		printk(KERN_INFO "Walk Calibration complete !!! \n");
		input_report_rel(ps_msp430->input_dev, REL_DIAL,
			CALIBRATION_COMPLETE+1);
		input_sync(ps_msp430->input_dev);
	}
	if (irq_status & M_CALIBRATION_COMPLETE_JOG) {
		printk(KERN_INFO "Jog calibration complete !!!\n");
		input_report_rel(ps_msp430->input_dev, REL_DIAL,
			CALIBRATION_COMPLETE+1);
		input_sync(ps_msp430->input_dev);
	}
	if (irq_status & M_SLEEP_DATA_READY) {
		printk(KERN_INFO "detected SLEEP_MODE\n");
		if (msp430_read_sleep_data(ps_msp430, &sleep_data) >= 0) {
			msp430_report_sleep_values(ps_msp430, &sleep_data);
		/* Hold a 500ms wakelock to let data get to LiveDataHandler */
			wake_lock_timeout(&ps_msp430->timed_lock, 0.5 * HZ);
		}
	}
...
static int msp430_check_for_motion(struct msp430_data *ps_msp430)
{
	...
	printk(KERN_INFO "%s()No motion detected (0x%x)\n", __func__, buff[0]);
...
static void msp430_passive_work_func(struct work_struct *work)
{
	...
	if (msp430_read_motion_data(ps_msp430, &curr) >= 0)
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case MSP430_IOCTL_GET_VERSION:
		printk(KERN_INFO "Switch to normal to get version\n");
		buff[0] = REV_ID;
		err = msp430_i2c_write_read(ps_msp430, buff, 1, 1);
		if (err >= 0) {
			err = (int)buff[0];
			printk(KERN_INFO "MSP software version %02x", buff[0]);
		}
		break;
	case MSP430_IOCTL_BOOTLOADERMODE:
		/* switch MSP to bootloader mode */
		printk(KERN_INFO "Switching to bootloader mode\n");
		msp430_stop_timers(ps_msp430);
		switch_msp430_mode(BOOTMODE);
		/* send password reset command to unlock MSP	 */
		printk(KERN_INFO "Password reset for reset vector 0x4400\n");
		...
		if (err < 0) {
			...
			printk(KERN_INFO "Default password reset\n");
		...
		if (err >= 0) {
			printk(KERN_INFO "Erasing the chip\n");
	...
	case MSP430_IOCTL_NORMALMODE:
		printk(KERN_INFO "Switching to normal node\n");
		/* switch MSP to normal mode */
		switch_msp430_mode(NORMALMODE);
		msp430_restart_timers(ps_msp430);
		check_msp_reset(ps_msp430);
		break;
	case MSP430_IOCTL_MASSERASE:
		printk(KERN_INFO " In MSP430_IOCTL_MASSERASE\n");
		msp430_build_command(MASS_ERASE, NULL, &cmdlen);
		err = msp430_i2c_write_read(ps_msp430, firm_cmdbuff,
					cmdlen, I2C_RESPONSE_LENGTH);
		break;
...
/* debug options for bosch */
	...
	case MSP430_IOCTL_SET_MONITOR_DELAY:
		...
		if (ps_msp430->mode != NORMALMODE)
			break;
		printk(KERN_INFO "set activity monitor delay = %d\n", delay);
		if ((ps_msp430->monitor_poll_interval ==  60000)) {
			cancel_delayed_work(&ps_msp430->monitor_work);
	...
	case MSP430_IOCTL_SET_MOTION_DELAY:
		...
		/* Clock mode - do nothing */
		if (ps_msp430->motion_poll_interval == 0)
			break;
		/* if not normal mode exit out of here */
		if (ps_msp430->mode != NORMALMODE)
			break;
		printk(KERN_INFO "set motion delay = %d\n",
			ps_msp430->motion_poll_interval);
		/* active mode supports 40 ms to 1 minute active mode udpates */
		if ((ps_msp430->motion_poll_interval >= 40) \
			&& (ps_msp430->motion_poll_interval < 60000)) {
			...
			wakeup_start_status_timer(ps_msp430->waketimer,
				ps_msp430->motion_poll_interval);
#endif
			printk(KERN_INFO "Enable actMode irq\n");

			/* Set active mode time interval */
			delay = (delay / 40);
			buff[0] = ACTV_TIMER;
			buff[1] = (unsigned char)((delay >> 24) & 0xff);
			buff[2] = (unsigned char)((delay >> 16) & 0xff);
			buff[3] = (unsigned char)((delay >> 8) & 0xff);
			buff[4] = (unsigned char)((delay) & 0xff);
			err = msp430_i2c_write(ps_msp430, buff, 5);
			if (err < 0)
				printk(KERN_INFO "Unable to enable actv timer interval\n");
			ps_msp430->intp_mask =
			...
			/*Set Gesture Mask*/
			buff[0] = GESTURE_MASK;
			ps_msp430->gesture_mask =
				ps_msp430->gesture_mask | GESTURE_TYPE_TAP;
			buff[1] = ps_msp430->gesture_mask;
			err = msp430_i2c_write(ps_msp430, buff, 2);
			if (err < 0)
				printk(KERN_INFO "Unable to enable actirq\n");

			if (msp430_read_motion_data(ps_msp430, &pedo) >= 0)
				msp430_report_pedometer_values(ps_msp430,
								&pedo);
			} else {
			...
			printk(KERN_INFO "Disable actMode irq\n");
	...
	case MSP430_IOCTL_SET_FACTORY_MODE:
		printk(KERN_INFO "set factory mode\n");
	...
	printk(KERN_INFO "Set profile with \n");
	if (copy_from_user(&user, argp, sizeof(user))) {
		printk(KERN_ERR "copy from user returned error\n");
		err = -EFAULT;
		break;
	}
	printk(KERN_INFO "age = %02x, sex = %02x, weight = %02x, \
	...
	printk(KERN_INFO "Equipment type = %d", byte);
	...
	printk(KERN_INFO "Get manual calibration status");
	...
	printk(KERN_INFO "Manual calibration: jog speed = %d", byte);
	...
	printk(KERN_INFO "Set user calibration table\n");
	...
	case MSP430_IOCTL_ERASE_CALIB:
	printk(KERN_INFO "Erase calibration table\n");
	buff[0] = ERASE_CAL;
	buff[1] = ERASE_CAL_VAL;
	err = msp430_i2c_write(ps_msp430, buff, 2);
	if (err < -1) {
		printk(KERN_ERR "Failed to erase calibration table\n");
		err = -EFAULT;
	}
	break;

	case MSP430_IOCTL_GET_PEDO_DATA:
	/* if not normal mode exit out of here */
	if (ps_msp430->mode != NORMALMODE)
		break;
	if (msp430_read_motion_data(ps_msp430, &pedo) >= 0)
	...
	case MSP430_IOCTL_SET_SCREEN_ON_GESTURE_STATUS:
	printk(KERN_INFO "Set screen on gesture status = %d", (int)argp);
	if (argp) {
		ps_msp430->gesture_mask =
			ps_msp430->gesture_mask | GESTURE_TYPE_SCREEN_ON;
	} else {
		ps_msp430->gesture_mask =
			ps_msp430->gesture_mask & ~GESTURE_TYPE_SCREEN_ON;
	}
	...
	err = msp430_i2c_write(ps_msp430, buff, 2);
	if (err < -1) {
		printk(KERN_ERR "Failed to set gesture mask\n");
		err = -EFAULT;
	}
	break;

	case MSP430_IOCTL_SLEEP_ANALYSIS:
	printk(KERN_INFO "Sleep analysis set to ");
	if (copy_from_user(&byte, argp, sizeof(byte))) {
		printk(KERN_ERR "Failed to read from user: sleep analysis\n");
		err = -EFAULT;
		break;
	}
	printk(KERN_INFO "0x%x\n", byte);
	buff[0] = SLEEP_ANALYSIS_ON;
	buff[1] = byte;
	err = msp430_i2c_write(ps_msp430, buff, 2);
	if (err < -1) {
		printk(KERN_ERR "i2c read error: sleep analysis\n");
		err = -EFAULT;
		break;
	}
	break;

	case MSP430_IOCTL_GET_SLEEP_DATA:
	if (msp430_read_sleep_data(ps_msp430, &sleep_data) >= 0) {
		msp430_report_sleep_values(ps_msp430, &sleep_data);
		/* Hold a 500ms wakelock to let data get to LiveDataHandler */
		wake_lock_timeout(&ps_msp430->timed_lock, 0.5 * HZ);
	...
	case MSP430_IOCTL_SET_ACC_ENABLE:
	if (copy_from_user(&flag, argp, sizeof(byte))) {
		printk(KERN_ERR "Failed to read from user: accel enable/disable \n");
		err = -EFAULT;
		break;
	}
	printk(KERN_INFO " accel enable flag = 0x%x\n", flag);
	break;
...
static int msp430_input_init(struct msp430_data *ps_msp430)
{
	...
	/* this is for sleep analysis */
	set_bit(REL_MISC,  ps_msp430->input_dev->relbit);
			   ...
			   const struct i2c_device_id *id)
{
	...
	/*clear the gesture mask*/
	/*Temporarily turn on the screen on gesture by default*/
	ps_msp430->gesture_mask = 0x02;
	...
	if (err < 0) {
		dev_err(&client->dev, "misc input init failed: %d\n", err);
		goto err5;
	}
