...
static ssize_t cyttsp_low_power_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp *ts = dev_get_drvdata(dev);

	return snprintf(buf, CY_MAX_PRBUF_SIZE,
		"%u\n", ts->low_power_mode_enabled);
}
static ssize_t cyttsp_low_power_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp *ts = dev_get_drvdata(dev);
	int retval = 0;
	...
	if (retval < 0) {
		pr_err("%s: Failed to convert value\n", __func__);
		goto cyttsp_low_power_mode_store_error_exit;
	}

	retval = ttsp_read_block_data(ts, CY_REG_BASE, sizeof(cmd), &cmd);
	if (retval < 0) {
		pr_err("%s: read fail on operational reg r=%d\n",
			__func__, retval);
		goto cyttsp_low_power_mode_store_error_exit;
	...
	retval = ttsp_write_block_data(ts, CY_REG_BASE, sizeof(cmd), &cmd);
	if (retval < 0) {
		pr_err("%s: I2C write fail set/clear low power mode (ret=%d)\n",
			__func__, retval);
		goto cyttsp_low_power_mode_store_error_exit;
	}

	ts->low_power_mode_enabled = value;
	pr_info("%s: touch low power mode=%d\n", __func__,
		(int) ts->low_power_mode_enabled);

cyttsp_low_power_mode_store_error_exit:
	mutex_unlock(&ts->data_lock);
	return size;
}
static DEVICE_ATTR(low_power_mode, 0666,
	cyttsp_low_power_mode_show, cyttsp_low_power_mode_store);
...
static void cyttsp_ldr_init(struct cyttsp *ts)
{
	...
	if (device_create_file(ts->dev, &dev_attr_low_power_mode))
		pr_err("%s: Cannot create low_power_mode\n", __func__);
...
static void cyttsp_ldr_free(struct cyttsp *ts)
{
	...
	device_remove_file(ts->dev, &dev_attr_low_power_mode);
