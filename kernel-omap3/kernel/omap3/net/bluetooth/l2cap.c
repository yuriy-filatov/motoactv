...
static void __l2cap_chan_add(struct l2cap_conn *conn, struct sock *sk, struct sock *parent)
{
	...
	if (sk->sk_type == SOCK_SEQPACKET) {
		...
		l2cap_pi(sk)->scid = l2cap_alloc_cid(l); Old Code */
...
static inline struct sock *l2cap_get_sock_by_cid(int state, __le16 cid,
                                                        bdaddr_t *src)
{
        struct sock *s;
        struct sock *sk = NULL, *sk1 = NULL;
...
static void __l2cap_sock_close(struct sock *sk, int reason)
{
	...
	case BT_CONNECT2:
		/* if (sk->sk_type == SOCK_SEQPACKET) { Old code */
                conn = l2cap_pi(sk)->conn;
                if ((sk->sk_type == SOCK_SEQPACKET ||
                                sk->sk_type == SOCK_STREAM) &&
                                conn->hcon->type != LE_LINK) {
...
static void l2cap_le_conn_ready(struct l2cap_conn *conn)
{
        ...
        sk = l2cap_sock_alloc(sock_net(parent), NULL,
                                BTPROTO_L2CAP, GFP_ATOMIC);
...
static inline void l2cap_check_encryption(struct sock *sk, u8 encrypt)
{
	...
	if (encrypt == 0x00) {
		if (l2cap_pi(sk)->sec_level == BT_SECURITY_MEDIUM ||
				l2cap_pi(sk)->sec_level == BT_SECURITY_HIGH) {
			BT_DBG("ce m/h %d", l2cap_pi(sk)->sec_level);
			l2cap_sock_clear_timer(sk);
			l2cap_sock_set_timer(sk, HZ * 5);
		}
	} else {
...
static int l2cap_security_cfm(struct hci_conn *hcon, u8 status, u8 encrypt)
{
	...
	for (sk = l->head; sk; sk = l2cap_pi(sk)->next_c) {
		...
		BT_DBG("scid %d", l2cap_pi(sk)->scid);
