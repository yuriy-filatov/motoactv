...
static struct cpuidle_params mapphone_cpuidle_params_table[] = {
	/* valid, sleep_latency, wake_latency, threshold (target_residency) */
	/* C1 */
	{1, 0, 12, 15},
	/* C2 */
	{1, 0, 18, 20},
...
int mapphone_set_l3_opp2_rate(unsigned long rate)
{
	int ret = -EINVAL;

	if (rate == S200M) {
		mapphone_omap3630_l3_rate_table[VDD2_OPP1].rate = S100M;
		mapphone_omap3630_l3_rate_table[VDD2_OPP2].rate = S200M;
		ret = 0;
	} else if (rate == S166M) {
		mapphone_omap3630_l3_rate_table[VDD2_OPP1].rate = S83M;
		mapphone_omap3630_l3_rate_table[VDD2_OPP2].rate = S166M;
		ret = 0;
	} else {
		printk(KERN_ERR "Invalid L3 frequency requested (%ld)\n", rate);
	}

	printk(KERN_ERR "L3 rates configured to %ldMHz/%ldMHz\n",
		mapphone_omap3630_l3_rate_table[VDD2_OPP1].rate/1000000,
		mapphone_omap3630_l3_rate_table[VDD2_OPP2].rate/1000000);
...
#if defined(CONFIG_SND_SOC_CS42L73) || defined(CONFIG_SND_SOC_CS42L73_MODULE)
#define CS42L73_RESET_GPIO  15
#define AUDIO_CLOCK_REQ_GPIO 25

static int __init mapphone_cs42l73_init(void)
{
	...
	mdelay(3);
	gpio_direction_output(CS42L73_RESET_GPIO, 1);
	mdelay(3);
...
static struct i2c_board_info __initdata
	mapphone_i2c_bus2_master_board_info[] = {
	{
		...
		.platform_data = &mapphone_msp430_data,
	},
};
...
#endif
#if defined(CONFIG_LEDS_LM3535)
	{
		I2C_BOARD_INFO("lm3535", 0x38),
	},
#endif
...
/*	mapphone_camera_init();*/
	mapphone_touch_init(&mapphone_i2c_bus1_master_board_info[0]);
	mapphone_audio_init();
	usb_musb_init();
	mapphone_ehci_init();
	mapphone_sdrc_init();
